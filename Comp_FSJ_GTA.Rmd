---
title: "Comparative analysis between GTA dataset and FishstatJ dataset"
output:
...
Information on fishing gear and fishing method (fishing_mode) is available in GTA but not in FSJ. The tRFMO is not displayed as well but as the Spatial Area is much precise in FSJ it can be retrieved (excepted one part in the Pacific Oean that is overlapped between two tRFMOs areas).
...
The FSJ dataset is much more exhaustive for species and flags. A greater number of species can be observed in FSJ, 347 species, while GTA lists only 65. The fishing_fleet categories are also more numerous in FSJ, due to the higher level of detail, whereas in GTA some data are aggregated (for example, 'Réunion' is included in France).

```{r numberdim, tab.cap="Number of dimensions for each dataset, after mapping of fishing_fleet and species for FSJ"}
qflextable(onlyminortabletest)
```
...
GTA catch data begin in 1918, while FSJ data start in 1950, providing 32 additional years of historical coverage in the GTA dataset. Table \@ref(tab:numberdim))
...
# ggplot2::scale_fill_manual(
#   values = c(
#     "Both: GTA > FSJ" = "#1B9E77",  # vert bleut� fonc�
#     "GTA only"        = "#66C2A5",  # vert bleut� clair (m�me teinte)
#     
#     "Both: FSJ > GTA" = "#D95F02",  # orange fonc� (ou rouge brun)
#     "FSJ only"        = "#FC8D62",  # orange clair (m�me teinte)
#     
