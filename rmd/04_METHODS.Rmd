...
- **Digital Format Standardization:** The Comma-Separated-Value (CSV) format, adhering to the RFC 4180 standard, has been endorsed.
- **File Encoding Protocol:** Whether in CSV or JSON format, encoding is standardized to UTF-8.
- **Data Dictionary:** A robust data dictionary has been established to guide the structure and semantics of the fisheries data exchange format. This repository, containing terms for column headers and their definitive explanations, is accessible at [FDI Terms on GitHub](https://github.com/fdiwg/fdi-terms). Notably, a specific fisheries term isn't rigidly bound to a single reference code list but varies based on its application context.

In context-specific scenarios, like the Global Tuna Atlas, associations with particular code lists can be found at [this link](https://github.com/fdiwg/fdi-mappings/tree/main/global/firms/gta).

The Global Tuna Atlas has adopted the Generic (measurement-independent) data structure for its operations (\@ref(tab:cwpformat))

```{r cwpformatloading, include=FALSE}

CWP_format <- read_delim(here("inputs/CWP_format.csv"), 
    delim = ";", escape_double = FALSE, trim_ws = TRUE) %>% rename(" "="...1")

```


```{r cwpformat, echo=FALSE, results='asis'}

qflextable2(CWP_format, grouped_data = c(" ") )
...
Before combining the datasets, a pre-processing step is applied to three out of the five datasets to restructure their shape to follow the CWP standards (see \@ref(tab:preprocess)). This restructuring ensures uniformity in the format of the data, facilitating the subsequent integration process.
...
```{r preprocess, tab.cap = "Datasets and pre-processing functions used in the creation of the final dataset", results='asis'}

qflextable2(datasets_links_processing)

```

#### Mapping the data

Additionally, to ensure consistency and comparability across datasets, a harmonization process is performed to standardize the code lists for datasets not having be mapped yet. The harmonized datasets allow for seamless data integration and accurate analysis across different tRFMOs. 
...
For each step, a json is provided containing several sheets among which one recapitulate the inputs, the ouptputs and provide metadata in dublin core format. 

The steps are the following: 

Step 1: Creation of the Database model and loading of shapefiles and codelist. 

- Associated json: https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/tunaatlas_qa_dbmodel%2Bcodelists.json

- Entities: https://docs.google.com/spreadsheets/d/12zi49c5wd9Nc7N1anv3lCM4f66L-5x3-Xsh6ZGGc-ns/edit#gid=1316104870

Step 2: Loading of the mapping

- Associated json: https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/tunaatlas_qa_mappings.json

- Entities :https://docs.google.com/spreadsheets/d/1xzXgIxjQm81Fg7PCgtdiwhkB9sgtPKk-NByQDRtAGrE/edit#gid=1158322074

Step 3: Loading and Pre-processing of the dataset provided by CCSBT, IATTC, ICCAT, IOTC, WCPFC. 

Associated json: 
- https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/gta_datasets_catch_ccsbt.json
- https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/gta_datasets_catch_iattc.json
- https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/gta_datasets_catch_iccat.json
- https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/gta_datasets_catch_iotc.json
- https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/gta_datasets_catch_wcpfc.json

Entities: Presented earlier see \@ref(tab:preprocess) lines 1 to 5)

Step 4: Creation of the Nominal dataset 

Associated json: https://raw.githubusercontent.com/firms-gta/geoflow-tunaatlas/master/tunaatlas_qa_global_datasets_catch.json

Entities presented earlier see \@ref(tab:preprocess) line 6)

The Step 4 is the only to display data in zenodo.
