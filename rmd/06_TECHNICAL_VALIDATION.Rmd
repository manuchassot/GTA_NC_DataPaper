...
```{r}
table_comp <- read.csv(here("rmd/Comparison_NC_FS/figures/Diffstratas/Comparison.of.number.of.stratas.between.the.two.datasets.csv")) %>% rename("Nominal Catch" = "Nominal.catch", " " = X)
```


```{r tablecompNCFS, tab.cap = "Comparison"}
qflextable2(table_comp, columns_to_color  = c("Difference"))
```

```{r temporal-analysis, include=FALSE}
...
```{r tablecompNCFSfiltered , tab.cap = "Comparison of the main dimensions for FishstatJ and Nominal Catch"}
...
```{r value-on-time, fig.cap="Recorded value of catches from 1918 to 2021 for FishstatJ and Nominal Catch datasets"}
...
```{r graphdiffvaluetime, fig.cap="Relative differences in value of the recorded catches between FishstatJ dataset and Nominal Catch"}
...
```{r fig.cap = 'Relatives percentage of catches by species for Fishstat and Nominal catches dataset', eval=TRUE}
...
```{r fig.cap = 'Relatives percentage of catches by species group for Fishstat and Nominal catches dataset', eval=TRUE}
...
```{r barplot-fleet-label, fig.cap = 'Relatives percentage of catches by country for Fishstat and Nominal catches dataset'}
...
```{r}
data <- read.csv(here("rmd/Comparison_NC_FS/figures_filtered/Diffstratas/Difference.between.stratas.of.the.two.datasets.csv")) %>% dplyr::select(-measurement_unit) %>% dplyr::rename("Difference in %" = "Difference..in...")%>% dplyr::rename("Difference in value" = "Difference.in.value") %>% filter(Dimension == "fleet_label")

qflextable2(data, grouped_data = c("Dimension", "Loss...Gain"), columns_to_color  = c("Difference in value"))
```
...
diffstratas <- read.csv(here("rmd/Comparison_NC_FS/figures_filtered/Diffstratas/Difference.between.stratas.of.the.two.datasets.csv")) %>% dplyr::filter(Dimension %notin% c("year", "GRIDTYPE")) %>% dplyr::select(-measurement_unit) %>% dplyr::rename("Loss / Gain" = "Loss...Gain", "Difference in value" = "Difference.in.value")%>% dplyr::rename("Difference in %" = "Difference..in...") %>% filter(Dimension == "ocean_basin")
...
# #     knitr::include_graphics(here(file_path))
# # 
# #   }
# # }
