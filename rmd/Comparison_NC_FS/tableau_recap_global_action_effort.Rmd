...
```{r child="Setup_markdown.Rmd", include=FALSE}
...
# source(knitr::purl("Functions_markdown.Rmd", quiet=TRUE),child_env_last)
...
new_path <- file.path(fig.path, paste0("Report_on_", entity$identifiers[["id"]]))
...
# source(knitr::purl("Functions_markdown.Rmd", quiet=TRUE),child_env_comp)
...
new_path <- paste0(fig.path, "Comparison/initfinal_",gsub("Markdown/","",sub_list_dir_2[1]),"_", gsub("Markdown/","",sub_list_dir_2[length(sub_list_dir_2)]))
...
function_all_comp = function(counting, parameters = parameters_child, parameter_short = TRUE, sub_list_dir = sub_list_dir_2, child_env_baseinside =child_env_base){
  gc()
...
new_path <- paste0(fig.path, "Comparison/",gsub("Markdown/","",sub_list_dir[counting]),"_", gsub("Markdown/","",sub_list_dir[counting+1]))
...
qflextable2(ster, captionn = "Replacement code during the mapping codelist treatment", grouped_data = c("Dimension"), save_folder = "Summary")
...
qflextable2(not_mapped_total, captionn ="Summary of the not mapped factors and so replaced by 'UNK', '99.9', or 'NEI'", pgwidth = 6,  save_folder = "Summary")
