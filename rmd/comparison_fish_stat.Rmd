...
# csl: ../templates/dmk-format.csl # Use any CSL style. See https://www.zotero.org/styles for a good list. Ignored if citation_package: natbib
link-citations: TRUE
output:
  ...
  bookdown::word_document2: 
    template: ../templates/rdoc_template.docx
...
```{r eval = !child, child = 'setup_markdown.Rmd'}
...
```{r shapefilereading, include=FALSE, error=0, eval=FALSE}
...
knitr::knit_hooks$set(kable = function(before, options, envir) {
  if (before && is.null(options$tab.cap)) {
    filename <- paste0(knitr::opts_current$get("label"), ".html")
...
knitr::knit_hooks$set(qflextable = function(before, options, envir) {
  if (before && is.null(options$tab.cap)) {
    filename <- paste0(knitr::opts_current$get("label"), ".docx")
...
knitr::knit_hooks$set(ggplot = function(before, options, envir) {
  if (before) {
    # Get the chunk label (chunk name)
    chunk_label <- knitr::opts_chunk$get("label")

    # Check if the chunk label is not NULL
    if (!is.null(chunk_label)) {
      filename_pdf <- paste0(chunk_label, ".pdf")
      filename_png <- paste0(chunk_label, ".png")
      filename_html <- paste0(chunk_label, ".html")
...
knit_child_attention <- knitr::knit_child(text =c(
  ...
  '-***All the differences inferior to 0 corresponds to a loss in captures.***', 
  '\n',
...
cat(knitr::knit_child(text = c(
  ...
  '  captionn = paste0("Disappearing or appearing strata between ", titre_1, " and ", titre_2)',
  ')))',
  '',
  '```'
), envir = environment(), quiet= TRUE)
...
```{r }
pie_chart_2_default <- function (dimension, first, second = NULL, topn = 6, titre_1 = "first", 
...
```{r}
if(!unique_analyse){pie_charts_multiple <- lapply(list_pie_chart,FUN = function_pie_chart_df, first= init, second = final, topn = 4)}else{pie_charts_multiple <- lapply(list_pie_chart,FUN = function_pie_chart_df, first= init, topn = 5, title_yes_no = FALSE )}
...
```{r piechartlist, eval=!unique_analyse}

pie_chart_knit <- lapply(pie_charts_multiple, FUN = resume_knit_child)

```

```{r piechartlistprinting, eval=!unique_analyse, results='asis'}
cat(unlist(pie_chart_knit),  sep = "\n")
```

## Differnces by species 

```{r}

qflextable2(testspecies %>%rename(species = Precision, ValueFS = value_sum_1, ValueNC = value_sum_2) %>%  select(-c(Dimension, measurement_unit, number_lines1, number_lines2, loss, "Difference in number of lines")), captionn = "Differences between the two datasets focusing on species")
...
```{r capturesbyyeardiffcreationlist}
map_unit_knit_list <- lapply(map, FUN = function_knitting)
```

```{r capturesbyyeardiff, results='asis'}
cat(unlist(map_unit_knit_list), sep = "\n")
```
